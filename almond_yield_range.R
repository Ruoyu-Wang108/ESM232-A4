...
#' @author Ruoyu Wang, Naomi Tague
#' @references D.B. Lobell et al. Agricultural and Forest Meteorology 141 (2006) 208–218.
#' @return
#' almond yield (anomoly from California mean in ton/acre, standard deviation of yield anomalies over the sample years)
#'


almond_yield_range = function(clim,Tmincoeff1=-0.015, Tmincoeff2=-0.0046, Pcoeff1=-0.07, Pcoeff2=0.0043, intercep=0.28) {

  # extracted required climate variables

  tmp = clim %>%
    group_by(month, year) %>%
    dplyr::summarize(tmin_c=mean(tmin_c), .groups="drop")
  ...
  return(list(mean.yield=mean(yield), sd.yield = sd(yield)))
}
